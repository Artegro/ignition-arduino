...
void readrpm()  // расчитываем обороты.
{
  ...
  rpm = VMTtime;  // так как мы считаем все в микросекундах и ушли от градусов и оборотов , то для удобства , чтоб не переписывать программу , просто приравниваю переменную оборотов в времени вмт.
  MapTime = VMTtime + VMTtime / 4; // вычисляем время середены такта впуска.
  MapReadFlag = true;
}
...
void countIgnitionTime() { // Функция расчета времени сдвига зажигания.
  nextIgnition = VMTtime - ignitionDegree; // устанавливаем время выхода активного элемента из датчика , момент искры.
  ignitionFlagWithOffset = true;
  loadtime = VMTtime - ignitionDegree - Modul_Size; // устанавливаем время имитации шторки вхождение активного элемента в датчик
  loadRead = true;
 ...
 if (startloop > 1) { // проверяем переменную первого запуска , если оборот первый то пропускаем условия образования искры, до момента изменения показателей модулятора
  ...
  if (modulator != oldModulator) {
    ...
    timer0_micros = 0;  //Обнуляем таймер micros()
